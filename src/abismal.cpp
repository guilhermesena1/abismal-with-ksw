...
struct ReadLoader {
  ...
  static const uint32_t max_read_length;
};
...
// GS: constrained on 16-bit scoring and pre-allocation of various
// variables
const uint32_t ReadLoader::max_read_length = 1024;
...
inline bool
valid(const se_element &s, const AbismalAlign &aln,
      const double cutoff) {
  const uint32_t len = aln.get_query_aln_length();
  return
    valid_len(len, aln.q_sz) &&
    s.diffs <= valid_diffs_cutoff(aln.q_sz, cutoff);
}
...
struct pe_element {
  ...
  void reset(const uint32_t readlen1, const uint32_t readlen2) {
    ...
    r1.reset(readlen1);
    r2.reset(readlen2);
    max_aln_score = aln_params::max_pair_score(readlen1, readlen2);
  }
  void reset() {
    aln_score = 0;
    r1.reset();
...
valid_pair(const pe_element &best,
           const AbismalAlign &aln1, const AbismalAlign &aln2) {
  const uint32_t len1 = aln1.get_query_aln_length();
  const uint32_t len2 = aln2.get_query_aln_length();
  return
    valid_len(len1, aln1.q_sz) &&
    valid_len(len2, aln2.q_sz) &&
    best.diffs() <=
...
static void
align_se_candidates(const Read &pread, const Read &pread_rc,
                    se_candidates &res, se_element &best,
                    AbismalAlign &aln, string &cigar) {
  static const double cutoff = se_element::valid_frac;

  // exact match (one or more), no need to align
  if (res.has_exact_match()) {
    best = res.best; // ambig info also passed here
    aln.make_default_cigar();
    aln.inflate_cigar(cigar);
    return;
  }
  ...
  // sort candidates by pos so that empty ones are first
  ...
  const score_t max_scr = aln_params::max_single_score(readlen);

  for (; it != lim && it->empty(); ++it); //fast forward empty elements

  aln.encode_query(false, pread);
  aln.encode_query(true, pread_rc);
  for (; it != lim; ++it) {
    if (valid_hit(*it, readlen)) {
      cand_pos = it->pos;
      const score_t cand_scr = aln.ksw_score(it->diffs, it->rc(), it->rc()^it->elem_is_a_rich(), cand_pos);
  ...
  if (best.pos != 0) {
    ...
    if (aln.ksw_full(best.rc(), best.elem_is_a_rich()^best.rc(),
                 best.diffs, max_diffs, best.pos)) {
      aln.inflate_cigar(cigar);
      best.pos += aln.r.tb; // soft-clip start

      // convert from aln score to edit distance
      best.diffs = aln.edit_distance();

      // do not report and count it as unmapped if
      // edit distance is too much
      if (!valid(best, aln,cutoff)) best.reset();
    }
    else best.reset();
  }
  else best.reset();
...
map_single_ended(const bool VERBOSE, const bool allow_ambig,
  ...
  const vector<uint32_t>::const_iterator
    counter_st(begin(abismal_index.counter));
  const vector<uint32_t>::const_iterator
    counter_t_st(begin(abismal_index.counter_t));
  const vector<uint32_t>::const_iterator
    counter_a_st(begin(abismal_index.counter_a));

  const vector<uint32_t>::const_iterator
    index_st(begin(abismal_index.index));
  const vector<uint32_t>::const_iterator
    index_t_st(begin(abismal_index.index_t));
  const vector<uint32_t>::const_iterator
    index_a_st(begin(abismal_index.index_a));
...
#pragma omp critical
    ...
    for (size_t i = 0; i < n_reads; ++i) {
      ...
      aln.reset(reads[i].size());
      bests[i].reset();
      if (!reads[i].empty()) {
        //encode read in 4 bits
        ...
        if (!res.sure_ambig) { // avoid work if not needed
          revcomp_inplace(reads[i]);
          prep_read<!conv>(reads[i], pread_rc);
          revcomp_inplace(reads[i]);
        ...
        align_se_candidates(pread, pread_rc, res, bests[i], aln, cigar[i]);
...
#pragma omp critical
    ...
    for (size_t i = 0; i < n_reads; ++i) {
      ...
      if (!reads[i].empty()) {
        ...
        revcomp_inplace(reads[i]);
        if (!res.sure_ambig) {
          prep_read<t_rich>(reads[i], pread_t_rc);
        ...
        // T-rich, - strand
        if (!res.sure_ambig) {
          prep_read<a_rich>(reads[i], pread_a_rc);
        ...
        align_se_candidates(pread_t, pread_t_rc, res, bests[i], aln, cigar[i]);
        if (format_se(allow_ambig, bests[i], abismal_index.cl, reads[i],
              names[i], cigar[i], out_stream) == map_unmapped)
          bests[i].reset();
      }
...
best_pair(const pe_candidates &res1, const pe_candidates &res2,
  ...
  aln1.encode_query(false, pread1); // pread1 is always on + strand
  aln2.encode_query(true, pread2); // pread2 is always on - strand
  for (; j2 != j2_end; ++j2) {
    s2 = *j2;
    scr2 = 0;
    ...
    for (; j1 != j1_end && j1->pos + pe_element::min_dist <= lim; ++j1, ++a1) {
      ...
      if (scr2 == 0) { // ensures elements in j2 are aligned only once
        scr2 = aln2.ksw_score(s2.diffs, true, !s2.elem_is_a_rich(), s2.pos);
      }

      if (*a1 == 0) { // ensures elements in j1 are aligned only once
        scr1 = aln1.ksw_score(s1.diffs, false, s1.elem_is_a_rich(), s1.pos);
        *a1 = scr1;
      }
  ...
  if (best_pos1 != 0) { // a new better alignment was found
    ...
    aln1.ksw_full(false, s1.elem_is_a_rich(), s1.diffs, max_diffs1, best_pos1);
    aln1.inflate_cigar(cig1);
    s1.pos += aln1.r.qb; // soft-clip start
    s1.diffs = aln1.edit_distance();

    // re-aligns pos 2 with traceback
    aln2.ksw_full(true, !s2.elem_is_a_rich(), s2.diffs, max_diffs2, best_pos2);
    aln2.inflate_cigar(cig2);
    s2.pos += aln2.r.qb;
    s2.diffs = aln2.edit_distance();

    // last check if, after alignment, mates are still concordant
    const uint32_t frag_end = best_pos2 + aln2.r.qe;
  ...
  if (res1.should_align() && res2.should_align()) {
    ...
    res2.prepare_for_mating();
      best_pair<swap_ends>(res1, res2, pread1, pread2,
          cig1, cig2, mem_scr1, aln1, aln2, best);
  }
...
map_fragments(const uint32_t max_candidates,
  ...
  if (!read1.empty() && !read2.empty()) {
    ...
    return select_maps<swap_ends>(pread1, pread2, cigar1, cigar2,
      res1, res2, mem_scr1, res_se1, res_se2, aln1, aln2, best);
  }
  return false;
}
...
map_paired_ended(const bool VERBOSE,
  ...
  AbismalAlign aln1(genome_st);
  AbismalAlign aln2(genome_st);
    ...
    for (size_t i = 0 ; i < n_reads; ++i) {
      ...
      if (
        !map_fragments<conv, false,
          ...
          cigar1[i], cigar2[i], aln1, aln2, res1, res2, mem_scr1,
        ...
        !map_fragments<!conv, true,
          ...
          cigar2[i], cigar1[i], aln2, aln1, res2, res1, mem_scr1,
      ...
      if (!valid_pair(bests[i], aln1, aln2))
        bests[i].reset();

      if (!bests[i].should_report(allow_ambig)) {
        /*
        align_se_candidates(pread1, pread1_rc,
                            res_se1, bests_se1[i], aln1, cigar1[i]);

        align_se_candidates(pread2, pread2_rc,
                            res_se2, bests_se2[i], aln2, cigar2[i]);*/
      }
...
#pragma omp critical
    ...
    for (size_t i = 0 ; i < n_reads; ++i) {
      ...
      if (!bests[i].should_report(allow_ambig)) {
        align_se_candidates(
          pread1_t, pread1_t_rc,
          res_se1, bests_se1[i], aln1, cigar1[i]
        );

        align_se_candidates(
          pread2_t, pread2_t_rc,
          res_se2, bests_se2[i], aln2, cigar2[i]
        );
      }
      select_output(allow_ambig, abismal_index.cl,
